-- This is a CRT-interlaced glsl shader implementation from the bsnes project.
...
videomode {
	...
	vertex_source = [[
        ...
        void main()
        {
                ...
                ilfac = vec2(1.0,floor(solarus_io_size.y/200.0));

                // The size of one texel, in texture-coordinates.
                one = ilfac;

                // Resulting X pixel-coordinate of the pixel we're drawing.
                mod_factor = texCoord.x;
        }
	]],
	...
	fragment_source = [[
        ...
        #       define TEX2D(c) pow(TEXEL(solarus_sampler, (c)), vec4(CRTgamma))
        #else
        #       define TEX2D(c) TEXEL(solarus_sampler, (c))
        #endif
        ...
        vec2 transform(vec2 coord)
        {
                ...
                return (bkwtrans(coord)/overscan/aspect+vec2(0.5)) * solarus_io_size;
        }
        ...
        void main()
        {
                ...
                vec2 ilvec = vec2(0.0,ilfac.y > 1.5 ? mod(float(solarus_display_time)/10.0,2.0) : 0.0);
                vec2 ratio_scale = (xy - vec2(0.5) + ilvec)/ilfac;
        #ifdef OVERSAMPLE
                ...
                xy = floor(ratio_scale)*ilfac + vec2(0.5) - ilvec;
